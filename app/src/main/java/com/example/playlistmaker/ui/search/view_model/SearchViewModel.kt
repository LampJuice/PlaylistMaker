...
class SearchViewModel(
            ...
            historyInteractor.getHistory(object : SearchHistoryInteractor.HistoryConsumer {
                override fun consume(searchHistory: List<Song>?) {
                    if (!searchHistory.isNullOrEmpty()) {
                        searchLiveData.postValue(SearchUIState.ShowHistory(searchHistory))
                    } else {
                ...
                handler.post {
                    ...
                    if (songs.isEmpty()) {
                        ...
                        searchLiveData.value =
                            SearchUIState.ShowSearchResults(songs.toList())
                    }
                }
            }
    ...
    fun onTrackClick(song: Song): Boolean {
        if (!clickDebounce()) return false
        historyInteractor.saveTrack(song)
        return true
    }
